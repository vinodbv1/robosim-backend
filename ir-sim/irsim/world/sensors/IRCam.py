...
from shapely import MultiLineString, Point, is_valid, prepare, area
...
class IRCam:
    ...
    def __init__(
        ...
        obj_id: int = 0,
        range_min: float = 0.1, #shortest distance camera can see
        range_max: float = 7, #distance camera can see 
        FOV: float = 1.62173,  #radians (converted from 70 degrees)
        ...
        **kwargs,
        ...
        """
        Initialize the ThermalCamera sensor.


        """
        if offset is None:
            offset = [0, 0, 0]
        self.sensor_type = "camera_thermal"
        ...
        self.color = kwargs.get("color", "yellow")   # blue for camera
        ...
        for geom_index in potential_geometries_index:
            ...
            if obj.shape == "human":
                ...
                if camera_geometry.intersects(geo):
                    ...
                    human["x"] = rel_x + np.random.normal(0, 1/(10000*area(geo)))
                    human["y"] = rel_y + np.random.normal(0, 1/(10000*area(geo)))
                    human["theta"] = rel_theta + np.random.normal(0, 1/100000*area(geo))
